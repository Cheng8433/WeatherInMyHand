...
- 综合/空气质量接口返回的是 `weather_data` 实体快照，字段见下表。
...
| `cloud` | `String` | 云量（%） |
| `dew` | `Double` | 露点温度（℃） |
| `aqi` | `Integer` | 兼容旧字段，同 `aqi_us` |
| `aqi_us` | `Integer` | 美国标准 AQI（us-epa） |
| `aqi_cn` | `Integer` | 中国标准 AQI（cn-mee） |
| `aqi_qa` | `BigDecimal` | QAQI 指数（和风自研）。**当前无写入方，恒为 null** |
...
> 每个城市只保留**一行**（按 `city_name` upsert，见 `WeatherService`），因此表大小由城市数决定而非请求数。
> 接口响应的 `data` 还会额外带一个**不入库**的 `hourlyForecast` 数组（24h 温湿度趋势），结构为 `[{ "fxTime": "2026-09-05T14:00+08:00", "temp": "25", "humidity": "60" }, ...]`。**只有 `/info` 会带它**。
