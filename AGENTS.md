...
| `POST` | `/api/location/save` | body `{latitude, longitude}` → reverse geocode; or `{cityName}` → geo lookup; upserts one row per city |

Note: there is deliberately **no** "get my location" endpoint. `GET /api/location/local` (no params, returned the single globally-newest row) was **deleted on 2026-09-12** — it had no owner key, so it could only ever answer "what city did the last user look up". Per-device "last city" lives in the client's `WeatherCache`; a server-side city list would need an account/owner identity first.

Note on coordinates: `POST /api/location/save` stores the **geocoded city's** coordinates (city centre), never the caller's raw lat/lon. The raw values are used only for that one reverse-geocode call. Location is 敏感个人信息 under PIPL, and weather only needs city-level granularity, so this is enforced **server-side** rather than trusting each client version to behave.
...
- `POST /api/location/save` is largely redundant for the app's own operation (the city cache self-fills via `getOrFetchLocation`), and `saveLocationFromApi`/`fetchLocationFromApi` duplicate most of `reverseGeocode` — two near-identical calls to the same QWeather endpoint. Not worth churning before there is a second consumer.
