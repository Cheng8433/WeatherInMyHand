...
- **`weather_data` is effectively write-only.** Nothing reads it to serve a response: `findTopByCityNameOrderByUpdateTimeDesc` exists only to be the upsert base, and the global-newest lookup was deleted along with `getCityNameFromLastWeather` (its only reachable-caller was an unreachable blank-city branch). So the table costs 3 full-row writes per cache miss and is the sole reason for the deferred unique-index gap. Dropping the persistence layer (keeping `locations`, which *is* load-bearing) would delete that gap rather than patch it — not done yet because it changes the API's `data.id`/`updateTime` provenance.
