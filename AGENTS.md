...
- **City favourites are device-local only** (`FavoritesStore` — SharedPreferences, an ordered list, cap `MAX_CITIES` = 12). The header ☆ opens `FavoritesDialog`; picking a city runs it through `searchWeatherByCity` (the "user explicitly named this city" path → on failure it falls back to *that city's own* cache, never a cross-city substitute), so it inherits the existing `uiApplySeq` arbitration and deliberately adds **no** second sequence counter. Two decisions not to "tidy up": the list is a plain `\n`-joined string rather than JSON because `org.json` throws in the local JVM tests and would kill the unit-testable layer (`parse`/`join`/`plus`/`minus` are Context-free; the public methods are thin shells — that split is also why `join` trims/skips blanks, matching `parse`); and when full, `add` **refuses** the new city instead of evicting the oldest — silently dropping a hand-picked city would be worse than saying "满了". Comparison is by raw city-name string, so a GPS fix that resolves to a district (黄浦) and a favourited city (上海) legitimately coexist as two entries.
...
- No global HTTP-error statuses (intentional, see contract note above). Tests exist but are **thin and unit-level only** — no instrumentation/UI tests, so the Activity and the `LocationHelper`/`LoadOverlay`/`PageRenderer`/`FavoritesDialog` wiring is still only verified by compiling and smoke-testing on a device.
